...
> 本文中の「開発基盤検討（31）」「KMP 化検討（32）」「実装計画（archive_23_*）」は役目を終えて削除済み。当時の名前のまま残しており、実体は git 履歴にある。
...
| T-38 | 2 つのテキスト入力ダイアログを整理する | 内部設計 | 低 | 対処不要 2026-09-01 | [詳細](#t-38) |
...
- 対応方針: 4 か所の引数を入れ替える
- 関連: ③ の冒頭でレビュアー 4 本を実戦で回す題材（開発基盤検討 §12）
...
- 関連: T-14 のレビューを機にオーナーが方針を決めた（KMP 化検討 §4 とその決定ログ）。T-17 の着手条件がこれで変わる
...
- 関連: KMP 化検討 §8 が「段 1 に入る前に Android 側で先に書く」と定めている。T-22 を同じ作業で閉じる
...
- 対応方針: 段 1 実装計画。Koin モジュールを層ごとに 3 本に分け、段 2 でファイルを移すだけで済む形にする
...
- 対応方針: 段 2 実装計画。Android のまま分割し、KMP 化は段 3 に残す
- 着手条件: T-19 の完了
...
- 対応方針: package と `namespace` をモジュール境界に揃える。対応表は KMP 化検討 §3 が正。**テストの package と import が変わる**ので、段 2 とは別の PR にする——段 2 の機械判定を濁らせない
...
- 関連: KMP 化検討 §3。T-20 のレビューで code-quality-reviewer が重複を指摘し、オーナーが導入時期を決めた

### T-32

- 背景: KMP ライブラリプラグインはホストテストが既定で無効で、開け忘れると件数が 0 のままビルドが緑になる。段 3 のステップ 5 では、3 モジュールが KMP になったことで `./gradlew testDebugUnitTest` が一致するモジュールを失い、**0 件で緑**になった（入口を `testAndroidHostTest` へ移して対処済み）。いま 122 件を守っているのは「人が数えている」ことだけで、段 3 実装計画の残りステップは source set の付け外しが最も多い区間に入る
...
- 対応方針: 段 3 実装計画。各モジュールで「KMP 化」と「`commonMain` への移送」を別のコミットに分ける——前者は同じコードが同じターゲットでコンパイルされるだけなので Android の挙動が構造的に変わりようがなく、壊れたときに原因が Gradle の構成かコードの移送かに必ず切り分かる。iOS はスタブと全画面の 2 回出す
- 着手条件: T-28a（build-logic の立ち上げ）が済んでいること。実際には計画のステップ 1 がそれにあたる
- 優先度の根拠: ③ の本体。段 4（iOS シェル）の前提
- 関連: KMP 化検討 §3〜§9・§11・§13。T-28a / T-28b を内包する
...
- **残った穴**: 本物の Room を通らない（[T-35](#t-35)）／実物の `.app` を起動しない（[T-46](./23_技術改善バックログ.md#t-46)）／ライセンス一覧の中身を見ていない（[T-39](#t-39)）
...
- 結末: **整理せずに消えた**。F-012 で画面 09 を条項どおりに作り直したとき、2 つのダイアログの唯一の呼び出し元だった旧カテゴリー画面ごと無くなった。名前を作るダイアログは 02b の `NewNameDialog` に一本化されている

### T-55

- 背景: Xcode の GUI で署名を設定すると `project.pbxproj` に焼き込まれ、xcconfig 側の指定が黙って無効になる（pbxproj のほうが強い）。このリポジトリは public なので、焼き込まれた値がそのままコミットされる経路でもある
- 対応方針: ビルド設定を `iosApp/Configuration/Config.xcconfig` に出し、手元ごとに変わる値は末尾で `#include?` する追跡外のファイルに置く。`scripts/check-ios-signing.py` が追跡ファイルを走査し、CI の `docs` ジョブと `.githooks/pre-commit` の両方で回す
- 完了時の状態: 検査は追跡している pbxproj / xcconfig と、署名の実体・`Local.xcconfig`・`xcuserdata/` の追跡を見る。**守るのは「Xcode が開いた拍子に焼き込む設定」だけ**で、それを越える防御は張らない（[log_23](./log_23_技術改善バックログ.md) 2026-09-02）。**鳴らすときも値は 1 文字も出さない**。塞げていない経路は [T-56](./23_技術改善バックログ.md#t-56)・[T-57](./23_技術改善バックログ.md#t-57) に切り出した
- 優先度の根拠: 実機ビルドを始めた時点で 2 回焼き込まれており、public リポジトリでは push した時点で取り返しがつかない
- 関連: [git 運用定義書](../仕様/16_git運用定義書.md) §1.4。判断の経緯は [log_23](./log_23_技術改善バックログ.md) の 2026-09-02
...
- **結果（2026-09-03）: 外すだけでは足りず、3 つ入れた。**
  - (1) 本番の DB の開き方を `commonMain` の `applyAppDatabaseOptions()` に 1 本化した。**着手して初めて `addMigrations` がどちらのプラットフォームにも無いことが分かった**——外した先で `Migration` を書いても本番に効かない穴が空いていた
  - (2) `RoomMigrationTest` に「移行できない DB は落ちてデータが残る」を足した。**本番の設定関数を通す**ので `fallbackToDestructiveMigration` を戻すと落ちる（例外側・データ側の両方が独立に落ちることを変異で実測）
  - (3) スキーマ version を **3 から 1 へ畳んだ**（オーナーの判断）。1・2・3 のどの区間にも Migration が無く、3 を起点にすると「この 2 世代は永久に開けない」という但し書きが残るため。**端末の DB は捨てて入れ直した**。経緯は[データモデル定義書の決定ログ](../仕様/log_14_データモデル定義書.md) 2026-09-03
  - version の二重管理は `APP_DATABASE_VERSION` の `const` で解いた（`@Database` は retention が BINARY で実行時に読めない）
- 関連: [T-25](./23_技術改善バックログ.md#t-25)（(c)(d) はこの作業で埋まった）／ [T-17](./23_技術改善バックログ.md#t-17)（スキーマ検証）／ [T-34](./23_技術改善バックログ.md#t-34)（`RoomMigrationTest` の driver 経路。開く側は埋まった）
...
- **結果（2026-09-07）: `ios` ジョブに `./gradlew compileKotlinIosArm64 linkDebugFrameworkIosArm64` を 1 ステップ足した**（`if: !cancelled()`）。[17](../仕様/17_テスト戦略定義書.md) §5 に守備範囲の条項を置いた。
  - **リンク 1 本では足りなかった**。`linkDebugFrameworkIosArm64` を持つのは framework を宣言する `:shared:ui` だけなので、射程が**その依存グラフに載るモジュール**に縛られる。無修飾の `compileKotlinIosArm64` を併記して、iosArm64 ターゲットを持つ全モジュールに一致させた
  - **§6 からは行を落とさず絞った**。この網が届くのは Kotlin 側までで、**Xcode の実機構成（device destination・`embedAndSign` の `iphoneos` 経路・署名）と Release framework は無防備のまま**。どちらも段 4 が実際に触る場所なので [T-44](#t-44) に預けた
  - 2 案の比較・変異の実測・`rebuy.kmp.ios` のコメントを触らなかった理由は[決定ログ](./log_23_技術改善バックログ.md) 2026-09-07
- 関連: [T-35](#t-35) のレビューで test-reviewer が指摘。[T-44](#t-44)（この検査を前提に着手する）
...
- 結果（2026-09-10）: `license_title` を `strings.xml` へ置き、`LicenseScreen` のタイトルとテスト 3 本（`NavigationTest`・`NavigationStateRestorationTest`・`LicenseLibrariesTest`）のハードコードを `Res.string` 経由に揃えた。**表示は 1 文字も変えていない。**
  ...
  - [T-44](#t-44) の段 4 Step 2 でオーナーが気づいて着手した

### T-44

- 背景: T-31（段 3）で iOS シミュレータに全画面が出るようになったが、**外枠は Compose 1 枚のまま**。③ の完了条件は「iOS シミュレータで全画面が動き、**外枠が SwiftUI のナビゲーションバーになっている**」なので、③ はまだ終わっていない。T-31 は段 3 だけを守備範囲にしていたため、閉じると段 4 が台帳から落ちる。**この条件は当初「タブバー・ナビゲーションバー」と書いていたが、④ で[画面定義書](../仕様/13_画面定義書.md) §1 が「タブは無い」と決めたので落とした**（[要求定義書の決定ログ](../仕様/log_11_要求定義書.md) 2026-09-07）
- 対応方針: 実装計画を `plan_23_kmp段4実装計画.md` に起こしてから着手する。段 3 と同じく、Android の挙動を変えないことを制約に置く
- **範囲は「外枠の載せ替え」と「バー越しの見え方」まで**（2026-08-31 は「Liquid Glass 前提の画面設計は ④ の範囲」として除外していたが、**④ が Android 先行で進んで決まらないまま終わったため、2026-09-07 のオーナー判断で段 4 に含めた**。経緯は[アーキテクチャ定義書の決定ログ](../仕様/log_15_アーキテクチャ定義書.md) 2026-09-07）。**中身の画面を SwiftUI で書き直すことはしない**——外枠を `NavigationStack` に載せ替えれば iOS 26 のシステムが Liquid Glass を描くが、**バーの背後をコンテンツが通らないと屈折する対象が無くただの半透明の板に見える**ので、Compose の中身がバーの下まで流れるところまでを 1 つの範囲にする
- **段 4 で足す Release framework に検査を追随させる**: `shared/ui/build.gradle.kts` の `binaries.framework` は Debug のみで、実機配布のとき Release を足す。そのとき CI の実機ステップ（[17](../仕様/17_テスト戦略定義書.md) §5）も Release へ伸ばす——[T-50](./closed_23_技術改善バックログ.md#t-50) が入れたのは Debug だけで、**Release は別コンパイル経路なので今の網では一切見ていない**
- 着手条件: **④ の後**（2026-08-31 オーナー判断）。④ で画面構成が変わると外枠は作り直しになるので、構成が固まってから載せ替える。網（[T-42](./closed_23_技術改善バックログ.md#t-42)）は ④ の前に入っている前提。**MVP の F は 2026-09-01 に、FB は 2026-09-06 にすべて closed になり、前提だった [T-50](./closed_23_技術改善バックログ.md#t-50) も 2026-09-07 に完了したので着手できる**
- 網について: 段 4 は iOS だけを触る変更なので **Android のテストが 1 件も網にならない**。それに加えて、**`shared/ui/src/iosTest/` の 16 ファイル中 10 ファイルが `ReBuyApp()` を直接描き、画面の同定を `TOP_APP_BAR_TITLE`、戻るを `BACK_BUTTON` で行っている**。外枠を SwiftUI に出すとこの 2 つのタグは Compose ツリーから消えるので、**既存の iOS の網も半分が書き換えになる**（特に `NavigationIosTest` は戻る矢印でしか検証していない）。SwiftUI 側に出た外枠を見る層は [T-46](./23_技術改善バックログ.md#t-46) として未着手
- 優先度の根拠: ③ の残り。⑤ のリリースはここを通る
- 関連: [T-42](./closed_23_技術改善バックログ.md#t-42) ／ [T-35](./closed_23_技術改善バックログ.md#t-35) ／ [T-39](./closed_23_技術改善バックログ.md#t-39) ／ [T-41](./23_技術改善バックログ.md#t-41)
- **結果（2026-09-13）: 7 つの Step をブランチ 1 本に積んで完了した。** 実装計画（`plan_23_kmp段4実装計画.md`）は規約どおり削除し、残す知見は移した先で生きている。
  - **外枠は SwiftUI の `NavigationStack` 1 段＋ツールバー**（案 C）。画面の切り替えは Compose の `NavDisplay` のまま。3 案の比較は[アーキテクチャ定義書の決定ログ](../仕様/log_15_アーキテクチャ定義書.md) 2026-09-13、**案 C で付かないもの**（スクロール端の効果・遷移中にバーが中身と一緒に動かないこと）は [13](../仕様/13_画面定義書.md) §6 の条項
  - **バーに出すものは Kotlin が 1 つの状態として渡す**。再発火を止める鍵の設計は `ReBuyToolbarBridge.kt` の KDoc が持つ
  - **中身をバーの下へ流す値は既定 0 の CompositionLocal で配る**。**Android では `Scaffold` がその top を 1 度も読まない**ので、Android 不変が実装の分岐ではなく契約で保証される（[log_23](./log_23_技術改善バックログ.md) 2026-09-12）
