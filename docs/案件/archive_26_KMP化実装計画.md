...
- 作成日: 2026-08-29
- 位置づけ: [KMP 化検討](../検討/32_KMP化検討.md) の実装手順。**段 0（Navigation 3 化）だけを本書に展開する**。**段 0 の完了（2026-08-29）をもって閉じた**（続きは[末尾](#続き)）。段 1（KMP 移植）と段 2（iOS シェル）は、前の段が完了した時点で本書に追記する——後の段の手順は前の段の結果に依存しており、先に書いても推測になるため
...
段 1（モジュール分割）は 27、段 2（KMP/CMP 移植）と段 3（iOS シェル）は前の段の完了時に 28・29 として立てる。

**本書の「段 1」「段 2」は当時の段番号**（[KMP 化検討](../検討/32_KMP化検討.md) §10 の改訂前）で、現在の段 2・段 3 を指す。
