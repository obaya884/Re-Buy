...
- 作成日: 2026-08-29
- 位置づけ: [KMP 化検討](../検討/32_KMP化検討.md) の実装手順。**段 0（Navigation 3 化）だけを本書に展開する**。**段 0 の完了（2026-08-29）をもって閉じた**（続きは[末尾](#続き)）。2026-08-29 に `26_KMP化実装計画.md` から改名——実装計画は台帳 23 の付属文書として親番号を継ぐ形にしたため。段 1（KMP 移植）と段 2（iOS シェル）は、前の段が完了した時点で本書に追記する——後の段の手順は前の段の結果に依存しており、先に書いても推測になるため
...
段 0 は 2026-08-29 に完了し、本書はここで閉じた。以降の段は 1 段 1 文書で立てる。

- 段 1（Koin 化）: [archive_23_kmp段1実装計画.md](./archive_23_kmp段1実装計画.md)
- 段 2（モジュール分割）: [archive_23_kmp段2実装計画.md](./archive_23_kmp段2実装計画.md)
- 段 3（KMP/CMP 移植）・段 4（iOS シェル）: 前の段の完了時に同じ形で立てる

**本書の「段 1」「段 2」は当時の段番号**（[KMP 化検討](../検討/32_KMP化検討.md) §10 の改訂前）で、現在の段 3・段 4 を指す。
