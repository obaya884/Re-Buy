...
| 1 | モジュールを 4 つに分ける。**Android のまま**で、KMP 化も DI の差し替えもしない | Android 同一挙動、既存テストが 1 行も変わらず緑、CI 緑。§11 の開発基盤の壊れはここで塞ぐ |
| 2 | KMP/CMP 移植（Hilt → Koin、Room KMP、`java.time` の置き換え、テストを `commonTest` へ）。iOS は Compose 全画面を出す薄い SwiftUI ホスト 1 枚 | Android 同一挙動、iOS シミュレータで同じ画面、`commonTest` 緑、CI が 4 ジョブで緑 |
| 3 | iOS の外枠を SwiftUI の `TabView` ＋ `NavigationStack` に差し替え、共有 backstack と同期する | iOS がネイティブのタブバー・ナビゲーションバーで動く。Android は無変更 |

**1 段で 1 つのことだけを変える**のがこの段取りの要点。Nav3 化・モジュール分割・KMP 移植はどれも挙動が変わりやすく、混ぜると落ちたときに原因を切り分けられない。

段 1 と段 2 を分けるのは、モジュール分割が「Gradle の構成変更」だけで完結し、既存のテストがそのまま網になるため。ここで CI とツール（§11）の壊れを先に片づけておけば、段 2 では KMP 移植そのものだけを見ればよくなる。

iOS のホストを段 2 に置いて段 3 へ送らないのは、§13 の未確認事項（AboutLibraries が CMP で動くか、Navigation 3 の CMP 対応の安定性）が **iOS で 1 回動かすまで潰せない**ため。移植を全部終えてから「CMP が iOS で動かない」と判明する経路を残さない。
...
| 2026-08-29 | モジュール分割を独立した段（段 1）にし、以降の段を繰り下げる | オーナー判断。分割と KMP 移植を同じ段でやると、CI とツールの壊れ（§11）と移植の失敗が同時に出て切り分けられない。分割だけなら既存テストがそのまま網になる |
