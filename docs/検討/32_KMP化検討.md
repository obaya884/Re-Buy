...
## 13.1 段 4 の実施時期（2026-08-31 判断）

**段 4（iOS の外枠を SwiftUI へ）は ④ の後に回す。** §14 の完了条件は変えないが、達成の順番だけを入れ替える。

- 段 4 は**利用者から見た価値を生まない**——iOS を配るのは ⑤ なので、外枠がネイティブかどうかは誰にも届かない
- ④ で画面構成が変われば**外枠は作り直し**になる
- ③ の残りを抱えたまま ④ へ進むことは、ロードマップの「各サブプロジェクトが単独で価値を残す」原則に反しない。段 3 は既に価値を残している

**代わりに、iOS の網（T-42）を ④ の前に置いた**（2026-08-31 完了）**。** ④ で作る機能は `commonMain` に入るのでそのまま iOS にも載る。段 3 では移送しかしていなくても iOS 固有の不具合を 1 件踏んだ（落とし穴 22）。機能追加はもっと踏む。
