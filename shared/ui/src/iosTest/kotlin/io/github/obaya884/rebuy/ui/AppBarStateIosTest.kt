 ...
 *
 * **他の網より 1 段下を見ている。** 他の `iosTest` は本番 iOS の構成（外枠を描かない）で走り、
 * 外枠の内容は橋を通って渡ってくる（`IosTestApp.kt`）。ここはその**手前の `ReBuyAppBarState` を
 * 直接見る**ので、描き手が入れ替わっても検証がそのまま残る。**Swift が受け取る契約はこの形**
 * （画面定義書 §6）。
 *
...
class AppBarStateIosTest {
     ...
     * `BACK_BUTTON` を押す網は Material の描き手を通る経路にしかなく、**本番の iOS には無い**ため。
